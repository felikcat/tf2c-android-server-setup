...
* Edit as NON-root (the 'servers' account as an example): `${HOME}/game_tf2classic_dedicated/run_dm.sh`
[source,sh]
----
#!/bin/sh

./srcds_linux -console -game tf2classic +map dm_2fort +servercfgfile "server_dm.cfg" +mapcyclefile "mapcycle_dm.txt"
----
...
* To run commands on the server, *rcon* has to be used.
- This would mean joining your server, and doing say, `rcon changelevel cp_dustbowl`.